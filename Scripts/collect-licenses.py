...
Runs as the "Collect Licenses" build phase of the Xrash target, so the
list is scanned from what this build actually links on every build, never a
hand-kept copy that drifts (adapted from Irisin's):
  ...
  2. code copied into the app's own targets — a Swift file whose header
     comment names a license ships that header;
  3. the local packages under Packages/ that carry a LICENSE, vendored
     sources inside them included;
  4. every package pinned in Package.resolved — its checkout under
     ...
     TreeSitter's notice, libarchive.xcframework the upstream BSD terms).
     A checkout with no such file ships the copyright header of its
     sources instead.
...
LicensesViewController in the app.
"""
...
def local_entries(project, homepage):
    ...
    for package in sorted(os.listdir(root)):
        ...
        for path in license_files(base):
            directory = os.path.dirname(path)
            # The package's own license is named for the package; a notice
            # deeper in is vendored source, named for the folder it sits in
            # (Sources/XrashSymbols/Vendor/<Name>/LICENSE).
            name = package if directory == base else os.path.basename(directory)
            url = f"{homepage}/tree/main/{os.path.relpath(directory, project)}"
...
def package_entries(project, source_packages):
    resolved_path = os.path.join(
        project, "Xrash.xcodeproj", "project.xcworkspace", "xcshareddata", "swiftpm", "Package.resolved"
    )
