...
declare -a late_libraries=(
    "libswiftXPC.dylib:16.0"          # iOS 27 SDK overlay; killed a shipped 0.1.6 on iOS 15
...
check_libraries() {
    local binary="$1" entry line library minimum
...
#    returns nil and the control draws nothing. It is source-level and nothing
#    here can see it: `make check` runs Scripts/check-symbol-availability.py
